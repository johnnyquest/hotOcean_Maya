...
//
//     ATTENTION: THIS IS A MODIFIED VERSION OF DREW WHITEHOUSE'S SOURCE CODE
//     PLEASE VISIT http://code.google.com/p/houdini-ocean-toolkit/ FOR THE
//     ORIGINAL VERSION OR GET MORE INFORMATION ON http://odforce.net/wiki/index.php/HoudiniOceanToolkit
//
...
// We would like to use locks to try and make the vex functions
// re-entrant, though I haven't managed to get it working that well at the
// moment. So, instead I just try to guard against threading
// problems. Note: the fftw code is explicitly *not* re-entrant when
// calculationg plans, and blitz does'nt have win32 support for its
// multithreading at the moment. (http://sourceforge.net/projects/loki-lib/)
//#include <loki/Threads.h>

// we use blitz for n-dimensional arrays (http://www.oonumerics.org/blitz/)
#include <blitz/array.h>

// includes a nice reentrant gaussian random number generator (http://www.openexr.com)/
...
namespace drw
{
    ...
    struct EvalData {  // code added by CS
        float disp[3];
        float normal[3];
    ...
    class OceanContext //: public Loki::SingleThreaded <OceanContext>
    {  
...
*/
        ...
        void alloc_disp_y()
        {
            ...
            _disp_y_plan = fftwf_plan_dft_c2r_2d(_M,_N,
                reinterpret_cast<fftwf_complex*>(_fft_iny.data()),
        ...
        void alloc_chop()
        {
            ...
            _disp_x_plan = fftwf_plan_dft_c2r_2d(_M,_N,
                reinterpret_cast<fftwf_complex*>(_fft_inx.data()),
                reinterpret_cast<my_float*>     (_disp_x.data()),
                FFTW_ESTIMATE);
            _disp_z_plan = fftwf_plan_dft_c2r_2d(_M,_N,
                reinterpret_cast<fftwf_complex*>(_fft_inz.data()),
        ...
        void alloc_jacobian()
        {
            ...
            _Jxx_plan = fftwf_plan_dft_c2r_2d(_M,_N,
                reinterpret_cast<fftwf_complex*>(_fft_injxx.data()),
                reinterpret_cast<my_float*>     (_Jxx.data()),
                FFTW_ESTIMATE);
            _Jzz_plan = fftwf_plan_dft_c2r_2d(_M,_N,
                reinterpret_cast<fftwf_complex*>(_fft_injzz.data()),
                reinterpret_cast<my_float*>     (_Jzz.data()),
                FFTW_ESTIMATE);

            _Jxz_plan = fftwf_plan_dft_c2r_2d(_M,_N,
                reinterpret_cast<fftwf_complex*>(_fft_injxz.data()),
                reinterpret_cast<my_float*>     (_Jxz.data()),
                FFTW_ESTIMATE);
        }


        void eval_uv(float u,float v, EvalData& data)
        {
            int i0,i1,j0,j1;
            float frac_x,frac_z;
            ...
            #define BILERP(m) (lerp(lerp(m(i0,j0),m(i1,j0),frac_x),lerp(m(i0,j1),m(i1,j1),frac_x),frac_z))
            {
                if (_do_disp_y)
                {
                    data.disp[1] = BILERP(_disp_y);
                }
                if (_do_normals)
                {
                    data.normal[0] = BILERP(_N_x);
                    data.normal[1] = BILERP(_N_y);
                    data.normal[2] = BILERP(_N_z);
                }
                if (_do_chop) 
                {
                    data.disp[0] = BILERP(_disp_x);
                    data.disp[2] = BILERP(_disp_z); 
                }
                else
                {
                    data.disp[0] = 0.0;
                ...
                if (_do_jacobian)
                {
                    compute_eigenstuff(BILERP(_Jxx),BILERP(_Jzz),BILERP(_Jxz), data);
                }                      
            }
            #undef BILERP
        }
        ...
        void eval2_uv(float u,float v, EvalData& data )
        {
...

            {
                if (_do_disp_y)
                {
                    data.disp[1] = INTERP(_disp_y) ;
                }
                if (_do_normals)
                {
                    data.normal[0] = INTERP(_N_x);
                    data.normal[1] = INTERP(_N_y);
                    data.normal[2] = INTERP(_N_z);
                }
                if (_do_chop) 
                {
                    data.disp[0] = INTERP(_disp_x);
                    data.disp[2] = INTERP(_disp_z); 
                }
                else
                {
                    data.disp[0] = 0.0;
                ...
                if (_do_jacobian)
                {
                    compute_eigenstuff(INTERP(_Jxx),INTERP(_Jzz),INTERP(_Jxz), data);
                }                      
            }
            #undef INTERP
        ...
        inline void compute_eigenstuff(const my_float& jxx,const my_float& jzz,const my_float& jxz, EvalData& data)   // code modified by CS
        {
            my_float a,b,qplus,qminus;
            a = jxx + jzz; 
            b = sqrt((jxx - jzz)*(jxx - jzz) + 4 * jxz * jxz);

            data.Jminus = 0.5f*(a-b);
            data.Jplus  = 0.5f*(a+b);
                    
            qplus  = (data.Jplus  - jxx)/jxz;
            qminus = (data.Jminus - jxx)/jxz;
            ...
            data.Eplus[0] = 1.0f/ a;
            data.Eplus[1] = 0.0f;
            data.Eplus[2] = qplus/a;

            data.Eminus[0] = 1.0f/b;
            data.Eminus[1] = 0.0;
            data.Eminus[2] = qminus/b;
        }

        void eval_xz(float x,float z, EvalData& data) // code modified by CS
        {
            assert(_Lx != 0 && _Lz  != 0);
            eval_uv(x/_Lx,z/_Lz, data);
        }
        void eval2_xz(float x,float z, EvalData& data) // code modified by CS
        {
            assert(_Lx != 0 && _Lz  != 0);
            eval2_uv(x/_Lx,z/_Lz, data);
        }
        ...
        void eval_ij(int i,int j,  EvalData& data  ) // code modified by CS
        {
            i = abs(i) % _M;
            j = abs(j) % _N;

            data.disp[1] = _do_disp_y ? _disp_y(i,j) : 0.0f;

            if (_do_chop)
            {
                data.disp[0] = _disp_x(i,j);
                data.disp[2] = _disp_z(i,j);
            }
            else
            {
                data.disp[0] = 0.0f;
            ...
            if (_do_normals)
            {
                data.normal[0] = _N_x(i,j);
                data.normal[1] = _N_y(i,j);
                data.normal[2] = _N_z(i,j);
            }
            if (_do_jacobian)
            {
                compute_eigenstuff(_Jxx(i,j),_Jzz(i,j),_Jxz(i,j), data);
            }                      
        }
    ...
    private:
        ...
        {
            //Lock lock(*this);
            _fft_inx.resize(_M,1 + _N/2);      
            _fft_iny.resize(_M,1 + _N/2);   
            _fft_inz.resize(_M,1 + _N/2);   

            // jacobian stuff
            _fft_injxx.resize(_M,1 + _N/2);      
            _fft_injxz.resize(_M,1 + _N/2);   
            _fft_injzz.resize(_M,1 + _N/2);   

            _htilda.resize(_M,1 + _N/2);

            assert(sizeof (*_fft_inx.data()) == sizeof (fftwf_complex));
            assert(sizeof (*_fft_iny.data()) == sizeof (fftwf_complex));
            assert(sizeof (*_fft_inz.data()) == sizeof (fftwf_complex));

            // jacobian
            assert(sizeof (*_fft_injxx.data()) == sizeof (fftwf_complex));
            assert(sizeof (*_fft_injxz.data()) == sizeof (fftwf_complex));
            assert(sizeof (*_fft_injzz.data()) == sizeof (fftwf_complex));
    ...
    class Ocean //: public Loki::SingleThreaded<Ocean>
    {
    ...
    public:
        ...
        {     
            ...
            { 
                _kx(i) = 2.0f * float(pi) * i / _Lx;
            }
            // the -ve components
            for (int i = _M-1,ii=0 ; i > _M/2 ; --i,++ii)
            {
                _kx(i) = -2.0f * float(pi) * ii / _Lx;
            }

            // the +ve components and DC
            for (int i = 0 ; i <= _N/2 ; ++i)
            { 
                _kz(i) = 2.0f * float(pi) * i / _Lz;
            }
            // the -ve components
            for (int i = _N-1,ii=0 ; i > _N/2 ; --i,++ii)
            {
                _kz(i) = -2.0f * float(pi) * ii / _Lz;
            }
            ...
            // this part cannot be parallized by OMP, cause of the gaussRand functions inside the loop!!!
            for (int i = 0 ; i  < _M ; ++i)
            {
                for (int j = 0 ; j  < _N ; ++j)
                {
                    ...
                    float chop_amount)
        {
            ...
            #pragma omp parallel for  // code added by CS
            for (int i = 0 ; i  < _M ; ++i)
            {
            ...
            r._fft_iny = scale * r._htilda;



            #pragma omp parallel sections  // code added by CS
            {
                #pragma omp section // code added by CS
                {
                ...
                #pragma omp section // code added by CS
                {
                    ...
                    if (do_chop && r._do_chop)
                    {
                        ...
                        {   
                            for (int j  = 0 ; j  <= _N / 2 ; ++j)
                            {     
                                r._fft_inx(i,j) = -scale * chop_amount * minus_i * 
                ...
                #pragma omp section // code added by CS
                {
                    if (do_chop && r._do_chop)
                    {
                        ...
                        {   
                            for (int j  = 0 ; j  <= _N / 2 ; ++j) 
                            {                    
                                r._fft_inz(i,j) = -scale * chop_amount * minus_i * 
                ...
                #pragma omp section // code added by CS
                {
                    if (do_jacobian && r._do_jacobian)
                    {
                        ...
                        for (int i = 0 ; i  < _M ; ++i)
                        {
                            for (int j  = 0 ; j  <= _N / 2 ; ++j)
                            {     
                                r._fft_injxx(i,j) = -scale * chop_amount * r._htilda(i,j) *  
                ...
                #pragma omp section // code added by CS
                {
                    if (do_jacobian && r._do_jacobian)
                    {
                        ...
                        for (int i = 0 ; i  < _M ; ++i)
                        {
                            for (int j  = 0 ; j  <= _N / 2 ; ++j)
                            {     
                                r._fft_injzz(i,j) = -scale * chop_amount * r._htilda(i,j) * 
                ...
                #pragma omp section // code added by CS
                {
                    if (do_jacobian && r._do_jacobian)
                    {
                        ...
                        for (int i = 0 ; i  < _M ; ++i)
                        {
                            for (int j  = 0 ; j  <= _N / 2 ; ++j)
                            {     
                                r._fft_injxz(i,j) = -scale *chop_amount * r._htilda(i,j) * 
            ...
            if (do_normal && r._do_normals)
            {
                ...
                for (int i = 0 ; i  < _M ; ++i)
                {
                    for (int j  = 0 ; j  <= _N / 2 ; ++j)
                    {     
                        r._fft_inx(i,j) = - plus_i * r._htilda(i,j) * _kx(i)  ;   
                    }
                }
                ...
                {   
                    for (int j  = 0 ; j  <= _N / 2 ; ++j) 
                    {                    
                        r._fft_iny(i,j) = - plus_i * r._htilda(i,j) * _kz(j) ;
                    }
                } 
